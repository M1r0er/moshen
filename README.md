...
它不产出正文。正文只有在你于写作页手动保存时才会写入磁盘；模型拿到的上下文用于讨论、诊断与整理，不会替你写下一章。对话中确实存在自动写入，但范围被限制在设定、大纲与知识库——它们会被标注来源，且不会覆盖你手工维护的内容。
...
| 页面 | 能做什么 |
|---|---|
| 对话 | 讨论大纲、人物、设定、伏笔与逻辑；流式输出；输入意图自动路由到合适的职能模型；发现问题时给出 L1 建议 / L2 质询 / L3 否决 |
